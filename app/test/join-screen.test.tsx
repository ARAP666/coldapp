...
describe('JoinScreen', () => {
  it('shows a loading state when codename is null', () => {
    ...
    const { getByTestId, getByText } = render(
      <JoinScreen codename={null} connected={false} onJoin={onJoin} onAbort={onAbort} />
    );
  ...
  it('displays the assigned codename once the server replies', () => {
    ...
    const onAbort = jest.fn();
    const { getByTestId, getByText } = render(
      <JoinScreen codename="BRAVO-07" connected onJoin={onJoin} onAbort={onAbort} />
    );
  ...
  it('disables the enter button when not ready (no codename or not connected)', () => {
    ...
    const { getByTestId } = render(
      <JoinScreen codename="BRAVO-07" connected={false} onJoin={onJoin} onAbort={onAbort} />
    );
